...
type FilePreview = { id: string; file: File; url: string };
...
export default function ProjectsTable({ initialProjects }: ProjectsTableProps) {
  ...
  function openEditModal(project: Project) {
    ...
    // clear "new" previews when switching edit target
    setFilePreviews((prev) => {
      cleanupPreviews(prev);
      return [];
    });
      ...
      if (imageFiles.length > 0) {
        for (const file of imageFiles) {
          ...
          if (!uploadRes.ok) {
            ...
            setFormErrors((prev) => ({ ...prev, images: msg }));
            setSaving(false);
            return;
          }
    ...
    return (
      <div className="flex flex-col gap-4">
        ...
        <div className="flex justify-between items-center">
          <span className="text-small">Total {filteredItems.length} projects</span>
  ...
  }, [filterValue, visibleColumns, filteredItems.length, onRowsPerPageChange, rowsPerPage, router]);
            ...
            <>
              ...
              <ModalBody>
                <Input
                  ...
                  errorMessage={
                    formErrors.name || `${formValues.name.trim().length}/100 characters`
                  }
                  onValueChange={(v) => {
                ...
                <Input
                  ...
                  onChange={(e) => {
                    ...
                    if (tooBig) {
                      setFormErrors((prev) => ({
                        ...prev,
                        images: `One or more images exceed ${MAX_MB}MB. Please choose smaller images.`,
                      }));
