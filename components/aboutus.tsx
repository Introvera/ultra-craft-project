  ...
  return (
    ...
    >
      <div className="mx-auto flex w-full max-w-7xl flex-col gap-12 lg:flex-row lg:items-center lg:gap-16">
        {/* LEFT: text (+ stats from md and up) */}
        ...
        {/* RIGHT: images / image + stats on mobile */}
        <div className="w-full md:basis-[45%]">
          {layout === "collage" ? (
            ...
            // SINGLE IMAGE layout (About page)
            <div
              className={`
                relative w-full h-[260px] sm:h-[320px] md:h-[360px] lg:h-[400px]
                overflow-hidden rounded-[32px] sm:rounded-[40px] lg:rounded-[48px]
                bg-card shadow-sm
                transition-all duration-[1500ms]
                ${
                  inView
      ...
      {/* On single layout, show stats below content on mobile as well */}
      {layout === "single" && <div className="md:hidden">{StatsBlock}</div>}
    </section>
  );
}
